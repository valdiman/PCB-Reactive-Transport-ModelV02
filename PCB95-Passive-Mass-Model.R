...
rtm.PCB95 = function(t, c, parms){
  ...
  Ct <- 83.53679 # ng/g PCB 95 sediment concentration
...
parms <- list(ro = 0.01, ko = 500) # Input reasonable estimate of ko and ro (placeholder values)
out.1 <- ode(y = cinit, times = t.1, func = rtm.PCB95, parms = parms) %>%
as.data.frame() -> out.1
}
